...
void Generator<stride::ContactType::Id::Workplace>::Apply(GeoGrid& geoGrid, const GeoGridConfig& ggConfig)
{
        ...
        for (const auto& range : size) {
            ...
            max_size = get<1>(range);
            AssertThrow(min_size < max_size,
                        "WorkplaceSizedGenerator> Invalid size range: " + to_string(min_size)
                        + ", " + to_string(max_size), m_logger);

            AssertThrow(min_size < max_size,
                        "WorkplaceSizedGenerator> Invalid min_size: " + to_string(last_size)
                        + ", " + to_string(min_size), m_logger);
        ...
        double sum_ratios = 0;
        double eps = 1.0e-03;
        AssertThrow(abs(1.0-sum_ratios) > eps,
                    "WorkplaceSizedGenerator> Sum of ratios not equal to 1", m_logger);
        */
}
