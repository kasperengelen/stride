...
from pystride.stride.pystride import Id_Daycare, Id_PreSchool
...
def vaccinate(simulator, event):
    if event.timestep == 7:
        ...
        for pIndex in range(pop.size()):
            age = pop[pIndex].GetAge()
            if pop[pIndex].GetPoolId(Id_Daycare) or pop[pIndex].GetPoolId(Id_PreSchool):
...
def plotNewCases(outputPrefix, iterations):
    ...
    plt.legend(["Daycare Kids are not vaccinated", "Daycare Kids are vaccinated"])
...
def runSimulation(outputPrefix, v, iteration, rng_seed):
    ...
    control.runConfig.setParameter("immunity_distribution_file", "data/lower_smoll_children_immunity.xml")
    control.runConfig.setParameter("immunity_link_probability", 0)   
    control.runConfig.setParameter("population_file", "data/DP_gengeopop.proto")
    control.runConfig.setParameter("geopopulation_file", "data/DP_gengeopop.proto")
...
def main():
    outputPrefix = os.path.join("DP", str(random.randint(0, 10000000)))
    iterations = 2
